...
path <- "C:/Users/User/Desktop/Python para Psicólogos/scripts-R/011 - Regressão Multinível/"
setwd(path)
...
rownames(ResultadoB) <- rownames(ResultadoA) <- c("Modelo Linear", "Modelo Nulo", "Modelo misto", 
                                                  "Modelo aleatorio", "Dois niveis", "Interacao")
ResultadoA
ResultadoB
...
plot(Models[[which.min(BICs)]], type = c("p", "smooth"),
     ylab="Res?duos", xlab="Estimativas")

# Scale location plot
plot(Models[[which.min(BICs)]], sqrt(abs(resid(.))) ~ fitted(.),type = c("p", "smooth"),
     ylab=expression(sqrt("|Res?duos|")), xlab="Estimativas")
